...
Peer = Fn.inherits('Develry.Peerpin.Common', function Peer(identity, connection, info) {
	...
	this.peerpin.blockchain.afterOnce('ready', function readyChain() {
		that.emit('initial_chain_state_request');
		that.talk('chain_state', that.peerpin.blockchain.length);
	});
});
...
Peer.setMethod(function processBlock(block_buffer, deemed_new) {
	...
	block = ChainfulNS.Block.fromBuffer(block_buffer, this.peerpin.blockchain.chainful, null, function gotBuffer(err) {
		...
		that.peerpin.blockchain.receivedPeerBlock(that, block, deemed_new);
	});
});
 ...
 * @param    {Function}   callback
 */
Peer.setTalkHandler('block_requests', function onBlockRequests(block_indexes, callback) {
	...
	for (i = 0; i < block_indexes.length; i++) {
		block = this.peerpin.blockchain.chainful.getByIndex(block_indexes[i]);
 ...
 * @param    {Function}   callback
 */
Peer.setTalkHandler('chain_state', function onBlockRequests(their_length, callback) {

	var that = this,
	    our_length = this.peerpin.blockchain.length,
	    indexes = [],
	    i;
	...
	if (our_length < their_length) {
		...
		this.peerpin.blockchain.requestBlocks(indexes, function gotBlocks(err, blocks) {

			if (err) {
				return console.error('Error requesting blocks: ' + err);
			}

			that.emit('chain_state_response', blocks);

			that.peerpin.blockchain.chainful.proposeBlocks(blocks).then(function() {
