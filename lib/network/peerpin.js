 ...
 * @return   {Buffer}
 */
Peerpin.setMethod(function serialize(value) {
	return Blast.Classes.Develry.Chainful.Chainful.serialize(value);
});
 ...
 * @return   {Object}
 */
Peerpin.setMethod(function unserialize(value) {
	return Blast.Classes.Develry.Chainful.Chainful.unserialize(value);
});
...
/**
 * Initialize & forward events
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.1.2
 * @version  0.1.2
 */
Peerpin.setMethod(function init() {

	var that = this;

	// Forward the added_block event
	this.blockchain.on('added_block', function onBlock(block) {
...
Peerpin.setMethod(function createDirectory(target_dir, callback) {
	...
	target_dir.split(sep).reduce(function(parent_dir, child_dir) {
		...
		if (cur_dir == '/' || cur_dir == 'C:\\' || cur_dir == 'c:\\') {
			return cur_dir;
		}
...
Peerpin.setMethod(function getFreePort(options, callback) {
	...
	test_server.on('error', function onError(err) {
