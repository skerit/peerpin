...
all_words = [
	...
	"speed", "dingus", "travel", "treat", "vein", "written", "cage", "chain",
...
// Count all the available words
word_count = all_words.length;

// Words used for input strings that are not dividable by 8
rest_words = [
	'',
	'ant',
	'big',
...
function encode(input) {
	...
	// Calculate the rest (the length should be dividable by 8)
	rest = input.length % 8;

	if (rest) {
		str = input.slice(0, 0-rest);
		rest_str = input.slice(0-rest);
	} else {
		str = input;
	}
	...
	for (i = 0; i < str.length; i += 8) {
		out = out.concat(getWords(str.substr(i, 8)));
	}

	if (rest) {
		out.push(rest_words[rest]);

		// Add the beginning string again
		// Just so we don't get repeating words at the end
		// when a lot of zeroes have to be added
		rest_str = input.slice(0, 8 - rest) + rest_str;

		out = out.concat(getWords(rest_str));
	}
...
/**
 * Get the 3 words for this hex string
 *
...
function getWords(hex_string) {
	...
	x = parseInt(hex_string, 16);

	w1 = (x % word_count);
	w2 = (Math.floor(x/word_count) + w1) % word_count;
	w3 = (Math.floor(Math.floor(x/word_count)/word_count) + w2) % word_count;
 ...
 * @return   {String}
 */
function getHex(w1, w2, w3) {

	var x;

	w1 = all_words.indexOf(w1);
	w2 = (all_words.indexOf(w2)) % word_count;
	w3 = (all_words.indexOf(w3)) % word_count;

	x = w1 + word_count * mn_mod((w2 - w1), word_count) + word_count * word_count * mn_mod((w3 - w2), word_count);
...
function decode(str) {
	...
	// Get the 4th last word
	rest = wlist[wlist.length - 4];

	if (rest && (rest = rest_words.indexOf(rest)) > -1) {
		// Get the 3 leftovers
		leftovers = wlist.splice(-3);

		// And pop off the rest count
		wlist.pop();
	}

	for (i = 0; i < wlist.length; i += 3) {
		out += getHex(wlist[i], wlist[i+1], wlist[i+2]);
	}

	if (leftovers) {
		leftovers = getHex(leftovers[0], leftovers[1], leftovers[2]);
		out += leftovers.slice(0-rest);
	}
